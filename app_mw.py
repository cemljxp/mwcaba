...
def update_temp(temp_avg, temp_max, temp_min, wind_avg, hum, hpa, sw_holiday, date_pick):
    temp_max_min = round(temp_avg,0) + 1
    temp_min_max = round(temp_avg,0) - 1
    # Se crea el vector con las variables predictoras
    val_pred = [[0,18,25,7,1012,78,14,1,0]]
    if sw_holiday == True:
        val_pred[0][0] = 1
    else:
        val_pred[0][0] = 0
    ...
    return [
    'Temperatura Promedio ({:.2f} °C)'.format(temp_avg),
    temp_max_min,
    temp_min_max,
    'Temperatura Máxima ({:.2f} °C)'.format(temp_max),
    'Temperatura Mínima ({:.2f} °C)'.format(temp_min),
    'Velocidad de Viento Promedio ({} km/h)'.format(wind_avg),
    'Humedad Relativa ({}%) '.format(hum),
    'Presión Atmosférica ({} hPa) '.format(hpa),
    wind_avg,
    hum,
    hpa,
    led_val,
...
def update_bars(swerr, err_sld):
    ...
    fig6.update_layout({'plot_bgcolor': 'black','paper_bgcolor': '#404040','font_color': 'white'}, margin=dict(l=30, r=30, t=150, b=20))
